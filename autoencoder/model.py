...
def build_convoultiona_ae(list_n_features_encoder=default_n_features_encoder,
    ...
    x_data_node = tf.compat.v1.placeholder(params.TF_DATA_TYPE, name='data')
    x_data_out_node = tf.compat.v1.placeholder(params.TF_DATA_TYPE, name='data')
    ...
    for l in range(n_convs_encoder):
        ...
        if l_type == 'c' or l_type == 'p':
            ...
            with tf.compat.v1.variable_scope(layer_name):
                conv_weight = tf.compat.v1.get_variable("weight",
                                              shape=[ksize, ksize,
                                                     n_feature_prev,
                                                     n_feature_next],
    ...
    ph_testing_error = tf.compat.v1.placeholder(dtype=tf.float32)
    op_assign_testing_error = tf.compat.v1.assign(testing_error, ph_testing_error)
    testing_psnr = tf.Variable(0.0, name='var_testing_psnr')
    ph_testing_psnr = tf.compat.v1.placeholder(dtype=tf.float32)
    op_assign_testing_psnr = tf.compat.v1.assign(testing_psnr, ph_testing_psnr)
    #########################################################
    ...
    # Add summaries
    #########################################################
    training_error_summary = tf.compat.v1.summary.scalar('training error', training_error)
    data_loss_summary = tf.compat.v1.summary.scalar('data_loss', data_loss)
    testing_error_summary = tf.compat.v1.summary.scalar('testing error', testing_error)
    testing_psnr_summary = tf.compat.v1.summary.scalar('testing psnr', testing_psnr)

    if with_wd:
        weight_decay_error_summary = tf.compat.v1.summary.scalar('weight decay error', weight_decay_term)
        summary_op_weight_decay = tf.compat.v1.summary.merge([weight_decay_error_summary])
    ...
    summary_op_training = tf.compat.v1.summary.merge([training_error_summary])
    summary_op_data_loss = tf.compat.v1.summary.merge([data_loss_summary])
    summary_op_testing = tf.compat.v1.summary.merge([testing_error_summary])
    summary_op_testing_psnr = tf.compat.v1.summary.merge([testing_psnr_summary])
