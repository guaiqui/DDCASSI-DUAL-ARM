...
def recon_snapshot(img_snapshot=[],
    ...
    #########################################################
    if img_snapshot is None or len(img_snapshot) == 0 or img_mask is None or len(img_mask) == 0:
    ...
    if gt_hs is not None and len(gt_hs) > 0:
            ...
            for i_admm in range(n_iters_ADMM):
                ...
                for i_nonlinear in range(n_iters):
                    ...
                    if i_nonlinear % 50 == 0:
                        ...
                        # compute PSNR
                        if do_summarize == True and gt_hs is not None and len(gt_hs) > 0:
    ...
    if out_filename is not None and len(out_filename) > 0:
