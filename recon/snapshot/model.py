...
def build_recon_network(list_n_features_decoder=[],
    ...
    # sparse_code_alpha = tf.compat.v1.placeholder(params.TF_DATA_TYPE, name='alpha')
    ...
    img_gt = tf.compat.v1.placeholder(params.TF_DATA_TYPE, name='img_gt')
    mask3d = tf.compat.v1.placeholder(params.TF_DATA_TYPE, name='mask3d')
    ...
    xk_ph = tf.compat.v1.placeholder(params.TF_DATA_TYPE, name='xk_ph')
    ...
    zk_ph = tf.compat.v1.placeholder(params.TF_DATA_TYPE, name='zk_ph')
    op_assign_zk = zk.assign(zk_ph)

    uk = tf.Variable(tf.ones(shape=img_gradient_shape) * 0.0001, name='uk', trainable=False)
    uk_ph = tf.compat.v1.placeholder(params.TF_DATA_TYPE, name='uk_ph')
    ...
    for l in range(n_convs_decoder):
        ...
        list_stride = [1, 1, 1, 1]
        pad = 'SAME'
        with tf.compat.v1.variable_scope(layer_name, reuse=is_reusable):
...
def build_recon_network_dual(list_n_features_encoder=[],
   ...
   # regularization on code
    ...
    summary_op_loss = tf.compat.v1.summary.merge([loss_summary])
